...
- **Plugin:** Use `balatro-mod-dev` plugin for shared patterns and game source
- **Rules:** See `.claude/rules/` for Lua coding style, mod conventions, and delegation guidelines
...
### Rule 7: Use Plugin for Common Knowledge
...
**Use the Task tool** to spawn sub-agents for research and code writing. The main agent (Sonnet) orchestrates; sub-agents execute and return recaps.
...
- Deep reasoning: `code-reviewer` (opus), `research-analyst` (opus), `strategic-planner` (opus)
...
Task(subagent_type="general-purpose", model="opus", prompt="[code-reviewer/research-analyst/strategic-planner template]")
```

**Delegation rules:**
- **Delegate to Sonnet** when: researching external sources, writing code >20 lines, exploring project architecture
- **Delegate to Opus** when: code review, synthesizing multi-source research, planning complex implementation
- **Delegate to Haiku** when: running temp scripts, simple data extraction
- **Do directly** when: small edits (<20 lines), presenting results, making simple decisions
- **Always include objective context** in Task prompts (WHY, not just WHAT)
...
The delegation rules above are the authoritative reference. `.claude/rules/delegation.md` provides a condensed version for quick lookup.
...
#### Phase 1: Research
1. Spawn Sonnet researchers to gather context:
   ```
   Task(subagent_type="Explore", model="sonnet", prompt="[research question with objective context]")
   ```
2. If multi-source research needed, synthesize with `research-analyst` (opus)

#### Phase 2: Plan
3. For complex tasks: spawn `strategic-planner` (opus) to design the implementation
4. For simpler tasks: enter plan mode directly (`EnterPlanMode`)
5. Exit plan mode (`ExitPlanMode`) — user reviews and approves

#### Phase 3: Execution
6. **Delegate to Sonnet `code-writer`** with approved plan:
   ```
   Task(
   ...
   ```
   For large plans, split into multiple sequential code-writer calls.

#### Phase 4: Review
7. Spawn `code-reviewer` (opus) to review the changes
8. Only act on issues with confidence ≥ 80
9. If CRITICAL issues: loop back to Phase 3
...
**Small changes** (typo fix, single-file edits) do NOT need plan mode — just do them directly or delegate to code-writer.
