...
## External Source Routing (CRITICAL)

Never use `Explore` or `general-purpose` for external sources — use the purpose-built researcher:
...
| Mod project structure | `balatro-mod-dev:project-explorer` |

Using `Explore` for these paths is blocked automatically by the `enforce-task-model` hook.
...
|---------------|-------|---------|
| `Explore` | **sonnet** | Mod project codebase only — NOT for external sources |
...
| `balatro-mod-dev:project-explorer` | **sonnet** | Mod-specific architecture analysis |
