...
2. **Copy script to project (if not exists):**
   ```bash
   cp "${CLAUDE_PLUGIN_ROOT}/scripts/fix_transparent_pixels.py" scripts/
   ```
...
| Command | Effect |
|---------|--------|
| `/balatro-mod-dev:fix-sprites assets/1x --preview` | Preview what would be fixed |
| `/balatro-mod-dev:fix-sprites assets/1x` | Fix all PNGs in assets/1x |
| `/balatro-mod-dev:fix-sprites assets --recursive` | Fix all PNGs in assets/ and subdirectories |
