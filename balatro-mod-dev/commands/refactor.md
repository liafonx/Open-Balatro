...
### 1. Gather Context
- Read `AGENTS.md` (root only) for repo structure, mod functionality, and constraints
...
3. **Code Review phase**:
   - After identifying targets, spawn `code-reviewer` (opus) to verify findings:
   ```
   Task(
     subagent_type="general-purpose",
     model="opus",
     prompt="[code-reviewer template]

   <objective>Review the refactoring candidates identified below for correctness and priority.</objective>

   <task>
   Review these files/sections and confirm which refactoring opportunities are genuine issues vs false positives.
   Files to review: [list]
   Candidates: [list]
   </task>"
   )
   ```
   - Only act on issues confirmed with confidence ≥ 80 by the code-reviewer

4. **Report phase**:
   - Present confirmed findings organized by priority
   - Ask user which items to address

5. **Refactor phase** (if user approves):
