...
<example>
Context: Main agent needs data from a script
user: 'Run this data extraction script'
assistant: 'I will invoke the script-runner agent to execute and return results.'
<commentary>Triggers when a temporary script needs to be executed for data.</commentary>
</example>"
model: haiku
color: blue
...
<workflow>
0. Read the `<objective>` section to understand WHY this script is being run
...
- **Files:** [relevant file references, if any]
- **Issues:** [problems encountered, if any]
- **Needs Review:** [what the main agent should verify]
</output_format>
