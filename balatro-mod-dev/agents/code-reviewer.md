...
## What to Save
- Project-specific anti-patterns found with confidence ≥ 80 (e.g., "this codebase consistently misses nil guard on G.GAME.hand before accessing hand.cards")
- Recurring Lua pitfalls specific to this project's coding style
- SMODS API misuse patterns confirmed in this mod

## What NOT to Save
- Individual review verdicts or per-session issue lists
- Style preferences or issues that were already fixed
- Pre-existing issues flagged as out-of-scope in prior reviews

## When to Invalidate
- After a major refactor addressing a recorded anti-pattern — remove the entry
- If a pattern no longer appears in the codebase, remove or archive it

## Format
- Group by category: Lua Pitfalls, SMODS Misuse, Nil Guards, Performance
- One entry per pattern; cite a representative file:line if available
- Remove entries when the underlying code is fixed
</memory_guidelines>
