...
1. Copy templates from plugin folder:
   ```bash
   cp "${CLAUDE_PLUGIN_ROOT}/scripts/sync_to_mods.template.sh" ./scripts/sync_to_mods.sh
   cp "${CLAUDE_PLUGIN_ROOT}/scripts/create_release.template.sh" ./scripts/create_release.sh
   chmod +x ./scripts/*.sh
   ```
